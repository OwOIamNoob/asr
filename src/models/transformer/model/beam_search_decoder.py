...
class BeamSearchDecoder(nn.Module):
    ...
    def __init__(self, decoder: Decoder, beam_size: int = 3) -> None:
        super(BeamSearchDecoder, self).__init__(decoder, beam_size)
    ...
    def _get_successor(
        ...
        """
        Handles sequences that follows a completed sequence.

        Args:
            currect_ps: Current beam search Probabilities.
            current_vs: Current beam search Vocabulary Indicies.
            finished_ids: (index of finished batch, index of finished sequence in that batch).
            num_successor: number of successors TO CONSIDER.
            eos_count: number of successors already ended w/ eos.
            k: beam_size.
        """
    ...
    def _get_predictions(self):
        ...
        for batch_idx, batch in enumerate(self.finished):
            ...
            # else, bring highest probability sentence
            else:
    ...
    def forward(
        ...
        decoder_inputs = torch.cat((decoder_inputs, self.ongoing_beams), dim=-1)  # batch_size * beam x 2
        # decoder_inputs = arrays of [<sos> , 1st output] per batch, per beam.

        # encoder_output.size = [batch_size, seq_len, d_model]
        encoder_dim = encoder_outputs.size(2)
        # encoder_outputs inflated to match decoder_inputs' size.
        ...
        for di in range(2, self.decoder.max_length):
            ...
            self.cumulative_ps = self.cumulative_ps.view(batch_size, self.beam_size)
            self.ongoing_beams = self.ongoing_beams.view(batch_size, self.beam_size, -1) # -1 ~ di-1
