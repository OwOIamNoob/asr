...
from diff_module import LinearAttention, Attention, LayerNorm
...
def get_attn_layer(in_dim, is_last, linear,  heads=4, dim_head=32):
    if is_last:
        return Residual(PreNorm(in_dim, Attention(in_dim, heads, dim_head)))
    elif linear:
        reutnr Residual(PreNorm(in_dim, LinearAttention(in_dim, heads, dim_head)))
    else:
        return nn.Identity()
...
class Unet(nn.Module):
    def __init__(self,
        ...
        # timestep settings
        timesteps_input_dim = hidden_dims[0]
        time_emb_dim = timestep_input_dim * 4
        
        self.time_embedding = nn.Sequential(nn.Linear(timesteps_input_dim, time_emb_dim),
                                            nn.SiLU(),
                                            nn.Linear(time_emb_dim, time_emd_dim))
        ...
        self.mid_attn = Resdidual(PreNorm(mid_dim, Attention(mid_dim)))
        self.mid_block2 = Residual(mid_dim, mid_dim, time_emb_dim)
        ...
        for idx, hidden_dim in enumerate(list(reversed(hidden_dims[:-1]))):
            is_last = idx >= (len(hidden_dims) - 2)
            up_blocks.append(nn.ModuleList([ResidualBlock(in_dim, in_dim, time_emb_dim),
                                              ResidualBlock(in_dim, in_dim, time_emb_dim),
                                              get_attn_layer(in_dim, is_last, use_linear_attn, heads=attn_heads, dim_head=attn_dim_head),
    ...
    def forward(self, sample, timesteps):
        ...
        time_emb = self.time_embedding(time_embedding)
        ...
        x = self.out_block(torch.cat(torch.cat((x, r), dim=1), time_emb))
        out = self.conv_out(x)
